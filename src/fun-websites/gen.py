...
{% block head %}
  <title>Fun Websites - Kevin Cao</title>
  <meta name="description" content="" />
{% endblock %}

{% block content %}
  <h1 class="text-center">Fun Websites</h1>
  <p>I sometimes stumble upon really unique websites on the Internet that I wish to share.</p> 
''')

print('<section>')
with open('/home/kjc/websites') as fp:
    for date, url in [
        l.strip().split(' ', maxsplit=2)
        for l in fp.readlines()
    ]:
        domain = re.match('https?://(.*)/', url).group(1)
        print(f'<a title="{date}" class="inline-block mx-6 my-3" href="{url}">{domain}</a>')
print('</section>')
